...
def prepare_data_for_lr(X, cat_cols):
    ...
    for col in cat_cols:
        if col in X_encoded.columns:
            # Cast to string to ensure consistency
...
def main(model_path):
    ...
    try:
        ...
        X_train_raw = pkg['X_train_all'] # Or reconstruction logic if needed
    ...
    # 4. Get TearSense Predictions for Comparison
    print("[3/5] Running TearSense Inference for Comparison...")
    try:
        # Assuming the PKL has a predict_proba method or we reconstruct
        # For this script, we assume we can extract the pipeline or infer
        # (Simplified: assumes pkg object is the predictor or has one)
        if hasattr(pkg, 'predict_proba'):
            ts_probs = pkg.predict_proba(X_test_raw)[:, 1]
        elif 'meta_model' in pkg:
            # Reconstruct prediction flow (simplified)
            # NOTE: This part depends on your specific `infer.py` logic.
            # If unavailable, we proceed with LR only.
            print("    (Warning: Could not run direct inference on PKL, skipping TS comparison curves)")
            ts_probs = None
        else:
            ts_probs = None
            
        # If test_preds were stored in result_holder during training, we could try to load them
        # But calculating fresh is safer.
    except:
        ts_probs = None
    ...
    for _, row in coef_df.iterrows():
        if abs(row['Coefficient']) > 0.0001: # Filter tiny coefs
